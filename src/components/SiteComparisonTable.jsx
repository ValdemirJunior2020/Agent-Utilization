...
export default function SiteComparisonTable({
  siteKPIs,
  selectedSite,
  setSelectedSite,
}) {
  ...
  const csvRows = rows.map((site) => ({
    ...
    ReportStartDate: site.reportStartDate,
    ReportEndDate: site.reportEndDate,
    ReportDateRange: site.reportDateRange,
    ReportDateCount: site.reportDateCount,
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          onClick={() => exportCsv("site-comparison.csv", csvRows)}
          className="no-print rounded-xl bg-hpNavy px-4 py-2 text-sm font-bold text-white"
        >
          Export Site CSV
        </button>
      </div>
        ...
        <table className="w-full min-w-[1200px] text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((site) => (
              <tr
                key={site.callCenter}
                onClick={() => setSelectedSite(site.callCenter)}
                className={`cursor-pointer hover:bg-sky-50 ${
                  selectedSite === site.callCenter ? "bg-sky-50" : "bg-white"
                }`}
              >
                ...
                <td className="px-4 py-3">
                  <div className="font-bold text-slate-800">
                    {site.reportDateRange || "Date not detected"}
                  </div>
                  <div className="text-xs text-slate-500">
                    Start: {site.reportStartDate || "N/A"} · End:{" "}
                    {site.reportEndDate || "N/A"}
                  </div>
                </td>

                <td className="px-4 py-3">
                  {site.reportDateCount || 0}
                </td>
