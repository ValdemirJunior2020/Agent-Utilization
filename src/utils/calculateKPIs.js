...
function siteStatus(site) {
  if (site.utilization < KPI_RULES.utilizationCritical || site.breakPct > KPI_RULES.breakCritical || site.offlinePct > KPI_RULES.offlineRisk) return 'Critical';
  if (site.utilization < KPI_RULES.utilizationReview || site.availablePct > KPI_RULES.availableRisk || site.onCallPct < KPI_RULES.onCallLoggedRisk) return 'Needs Review';
  return 'Healthy';
}
...
export function calculateAgentKPIs(rows) {
  return Object.entries(groupBy(rows, 'agent')).map(([agent, agentRows]) => {
    ...
    const loggedHours = sum(agentRows, 'loggedHours') || onCallHours + availableHours + breakHours + offlineHours;
...
export function calculateSiteKPIs(rows) {
  return Object.entries(groupBy(rows, 'callCenter')).map(([callCenter, siteRows]) => {
    ...
    const loggedHours = sum(siteRows, 'loggedHours') || onCallHours + availableHours + breakHours + offlineHours;
    const site = {
      callCenter,
      ...
      belowUtilizationTarget: agents.filter((agent) => agent.utilization < KPI_RULES.utilizationCritical).length,
      highBreakAgents: agents.filter((agent) => agent.breakPct > KPI_RULES.breakReview).length,
      highOfflineAgents: agents.filter((agent) => agent.offlinePct > KPI_RULES.offlineRisk).length,
      agents,
    };
...
export function calculateDailyTrend(rows) {
  const byDayAndSite = rows.reduce((acc, row) => {
    ...
    acc[key] = acc[key] || { date, callCenter: row.callCenter, onCallHours: 0, loggedHours: 0 };
  ...
  return Object.values(byDayAndSite)
    .map((item) => ({ ...item, utilization: safeDivide(item.onCallHours, item.loggedHours) }))
